...
function main(): void {
	...
	for (const spawnName in Game.spawns) {
		...
		const harvesters = roomRunner.getCreepsWithRole(MyCreep.ROLE_HARVESTER);
		const upgraders = roomRunner.getCreepsWithRole(MyCreep.ROLE_UPGRADER);
		const builders = roomRunner.getCreepsWithRole(MyCreep.ROLE_BUILDER);
		const repairers = roomRunner.getCreepsWithRole(MyCreep.ROLE_REPAIRER);
		const wallrepairers = roomRunner.getCreepsWithRole(MyCreep.ROLE_WALLREPAIRER);
		const miners = roomRunner.getCreepsWithRole(MyCreep.ROLE_MINER);
		const carriers = roomRunner.getCreepsWithRole(MyCreep.ROLE_CARRIER);
		const defenders = roomRunner.getCreepsWithRole(MyCreep.ROLE_DEFENDER);
